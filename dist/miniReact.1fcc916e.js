...
var _miniReact = require("./miniReact");
var _miniReactDefault = parcelHelpers.interopDefault(_miniReact);
const { createElement, render, useState } = (0, _miniReactDefault.default);
function Counter() {
...
},{"./miniReact":"9V7O5","@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}],"9V7O5":[function(require,module,exports,__globalThis) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
    ...
    return {
        ...
        props: {
            ...
            children: children.map((child)=>{
                return typeof child === "object" && child !== null ? child : createTextElement(child);
            })
        }
    };
}
...
function createDOM(fiber) {
    ...
    .filter((key)=>key !== "children").forEach((key)=>{
...
function reconcileChildren(wipFiber, elements) {
    ...
    while(index < elements.length || oldFiber != null){
        ...
        if (oldFiber && !sameType) {
            ...
            if (!wipRoot.deletions) wipRoot.deletions = [];
        ...
        index++;
        if (oldFiber) oldFiber = oldFiber.sibling //当处理完一个element和oldfiber时，移向下一个旧节点
        ;
    }
}
//执行函数式组件
function updateFunctionComponent(fiber) {
    ...
    ] //fiber.type指函数本身，执行函数组件，返回JSX对象，用数组包装
    ;
...
function performUnitOfWork(fiber) {
    ...
    ;
    if (isFounctionComponent) updateFunctionComponent(fiber);
...
function commitRoot() {
    if (wipRoot?.deletions) wipRoot.deletions.forEach(commitDeletion);
...
function commitDeletion(fiber) {
    ...
    } else commitDeletion(fiber.child);
}
...
function updateDom(dom, prevProps, nextProps) {
    //移除旧的事件监听器
    Object.keys(prevProps).filter((key)=>key.startsWith("on")).filter((key)=>!(key in nextProps) || prevProps[key] !== nextProps[key]) //事件被移除或监听函数被更换
    .forEach((name)=>{
        const eventType = name.toLowerCase().substring(2) //"onClick"->"click"
        ;
        dom.removeEventListener(eventType, prevProps[name]);
    });
    Object.keys(prevProps).filter((key)=>key !== "children").filter((key)=>!(key in nextProps)).forEach((key)=>{
        dom[key] = "";
    });
    Object.keys(nextProps).filter((key)=>key !== "children").filter((key)=>prevProps[key] !== nextProps[key]).forEach((key)=>{
...
},{}]},["iUuJv","fILKw"], "fILKw", "parcelRequiref402", {})

//# sourceMappingURL=miniReact.1fcc916e.js.map
